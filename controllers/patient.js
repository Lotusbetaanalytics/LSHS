...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  //check for user
  const admin = await Patient.findOne({ email: email }).select("+password");
...
exports.getMe = asyncHandler(async (req, res, next) => {
  const admin = await Patient.findById(req.admin.id);
  res.status(200).json({
    success: true,
    data: admin,
  });
...
exports.updateProfile = asyncHandler(async (req, res, next) => {
  ...
  const admin = await Patient.findByIdAndUpdate(req.admin.id, fieldsToUpdate, {
    new: true,
    runValidators: true,
  });
  res.status(200).json({
...
exports.updatePatient = asyncHandler(async (req, res, next) => {
  const admin = await Patient.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  });
  if (!admin) {
